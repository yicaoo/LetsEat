...
//  Modified by Yi Cao For The Use of Final Project (added additional extensions)
...
//Yi Added: For presenting generic alert
extension UIViewController {
    func addGenericAlert(message: String, alertTitle: String, actionTitle: String) {
    ...
    func addAlertCheckSettings(message: String, alertTitle: String, actionTitle: String) {
        let alert = UIAlertController(
            title: alertTitle,
            message: message,
            preferredStyle: .alert
        )
        alert.addAction(UIAlertAction(title: "Go to Settings", style: .destructive, handler: {action in
...
//Yi Added: For saving description notification center
extension Notification.Name {
    static let saveDescription = NSNotification.Name(rawValue: "saveDescription")
    static let dismissTimerPopover = NSNotification.Name(rawValue:"dismissTimerPopover")
    static let dismissContactPopover = NSNotification.Name(rawValue: "dismissContactPopover")
}

//Yi Added: For parsing MapKit returned phone number
extension String {
    func phoneNumberParser(inputString: String) -> String {
        var returnString = ""
        let indexStartOfText = inputString.index(inputString.startIndex, offsetBy: 1)
        let indexEndOfText = inputString.index(inputString.endIndex, offsetBy: -1)
        let preprocessedString =  String(inputString[indexStartOfText..<indexEndOfText])
        for character in preprocessedString {
...
{
    // this static lets us share a CMMotionManager
    // throughout our app
    // we want to share it because there's only one accelerometer
    // and one gyro
